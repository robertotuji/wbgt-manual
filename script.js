...
const translations = {
    ...
    en: { title: "WBGT Checker (Manual)", labelTemp: "Temperature (°C):", labelHum: "Humidity (%):", calc: "Calculate", clear: "Clear", mode: "Dark Mode", levels: ["Almost Safe", "Caution", "Alert", "Severe Alert", "Danger"] },
...
document.getElementById("calculate").onclick = () => {
    ...
    const label = translations[lang].levels[
        wbgt < 21 ? 0 : wbgt <= 25 ? 1 : wbgt <= 28 ? 2 : wbgt <= 31 ? 3 : 4
    ];
    const color = wbgt < 21 ? "#538DD5" : wbgt <= 25 ? "#C5D9F1" : wbgt <= 28 ? "#FFFF00" : wbgt <= 31 ? "#FFC000" : "#FF0000";

    const box = document.getElementById("result-box");
    box.classList.remove("hidden");
    box.style.backgroundColor = color;
    box.innerHTML = `<div>WBGT: ${wbgt}°C<br><strong>${label}</strong></div>`;
};
